...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        correct_softmax = softmax[range(N),y]
    
        data_loss = np.sum(-np.log(correct_softmax)) / N
        ...
        out1 = softmax.dot(W2.T) 
        # reverse the max() gate
        out1[z1 <= 0] = 0
        grads['W2'] = np.dot(h1.T,softmax) + reg * W2
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    #lr 1.000000e-03 reg 5.000000e-01 hs 200 lrd 8.500000e-01 train accuracy: 0.497122 val accuracy: 0.493000
